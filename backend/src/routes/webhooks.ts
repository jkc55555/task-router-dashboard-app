...
webhooksRouter.post("/postmark", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const mailboxHash = typeof body.MailboxHash === "string" ? body.MailboxHash : undefined;
    const subjectLen = typeof body.Subject === "string" ? body.Subject.length : 0;
    const hasAttachments = Array.isArray(body.Attachments) && body.Attachments.length > 0;
    console.log("[WEBHOOK] postmark received", {
      recipient,
      MailboxHash: mailboxHash ?? "(empty)",
      subjectLen,
      hasAttachments,
    });
    ...
    if (!config.enabled || config.provider !== "postmark") {
      ...
      console.log("[WEBHOOK] postmark rejected: config", {
        enabled: config.enabled,
        provider: config.provider,
        parseDomain: config.parseDomain ? "(set)" : "(empty)",
      });
    ...
    if (!parsed) {
      ...
      console.log("[WEBHOOK] postmark rejected: parse failed", {
        ...
        ToFullFirst: Array.isArray(body.ToFull) && body.ToFull[0] ? body.ToFull[0] : undefined,
      });
      return res.status(200).send();
    }
    ...
    if (!token) {
      ...
      console.log("[WEBHOOK] postmark rejected: no token", {
        ...
        hint: "Address must include +TOKEN, e.g. mailboxId+TOKEN@inbound.postmarkapp.com",
      });
      return res.status(200).send();
    }
