# Resolve failed migration `20250212000000_add_project_clarifying_and_waiting_fields`
...
1. **From your machine** (with Railway DB URL):
   ...
   npx prisma migrate resolve --rolled-back "20250212000000_add_project_clarifying_and_waiting_fields"
   ```
...
3. **Mark the failed migration as applied** so Prisma stops blocking:
   ...
   ```bash
   npx prisma migrate resolve --applied "20250212000000_add_project_clarifying_and_waiting_fields"
   ```
...
```bash
cd backend && DATABASE_URL="YOUR_RAILWAY_DATABASE_URL" npx prisma migrate resolve --rolled-back "20250212000000_add_project_clarifying_and_waiting_fields"
```
...
Using the **public** URL (or any URL that reaches the same DB as the container), run:
...
Or run the SQL in Railway’s Postgres **Query** tab (or any client connected to that database):
